...
function NavLogo() {
  ...
  return (
    <Link to="/" aria-label="Home">
      <div
        className="w-9 h-9 rounded-full p-[2px] transition-transform duration-300 hover:scale-110"
        style={{
          background: hovered
      ...
      >
        <div className="w-full h-full rounded-full bg-bg flex items-center justify-center">
          <span className="font-display italic text-[13px] text-text-primary select-none">AG</span>
        </div>
      </div>
    </Link>
  )
...
function SignUpButton() {
  ...
  return (
    ...
    >
      ...
      >
        Sign up <span className="text-muted">↗</span>
      </Link>
    </div>
  )
}
