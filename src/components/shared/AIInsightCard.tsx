 ...
 *
 * Shared component used by both the Member Dashboard and Admin Panel.
 * Import from this path in any new code:
 *   import AIInsightCard from '@/src/components/shared/AIInsightCard'
 *
 * The old path in components/dashboard/ re-exports this file for
 * backward compatibility with existing dashboard pages.
 */
...
export default function AIInsightCard({ insight }: { insight: AIInsight }) {
    ...
    const borderColor =
        insight.type === 'risk' ? 'var(--db-loss)' :
        insight.type === 'opportunity' ? 'var(--db-profit)' :
        insight.type === 'alert' ? 'var(--db-warning)' :
        'var(--db-info)'
